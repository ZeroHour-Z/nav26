    ...
    void publishOdometry(capture_Image_List_t* stream, OdometryType odom_type, bool show_path, bool show_camerapose) {
            ...
            switch(odom_type) {
                ...
                case OdometryType::TRANSFORM:
                    {
                    // SDK gives the relocalization correction as the pose of `map` in `odom`
                    // frame (i.e. T_odom_map). The standard ROS / Nav2 TF tree convention
                    // is map -> odom -> base_link, which requires T_map_odom = inverse(T_odom_map).
                    // We invert the pose here and publish it with parent=map, child=odom so the
                    // resulting TF tree is consistent and Nav2 costmaps can resolve `map`.
                    Eigen::Quaterniond q_om(
                        msg.pose.pose.orientation.w,
                        msg.pose.pose.orientation.x,
                        msg.pose.pose.orientation.y,
                        msg.pose.pose.orientation.z);
                    if (q_om.norm() < 1e-9) {
                        q_om = Eigen::Quaterniond::Identity();
                    } else {
                        q_om.normalize();
                    }
                    Eigen::Vector3d t_om(
                    ...
                    Eigen::Quaterniond q_mo = q_om.conjugate();
                    Eigen::Vector3d t_mo = -(q_mo * t_om);

                    geometry_msgs::msg::TransformStamped transformStamped;
                    transformStamped.header.stamp = msg.header.stamp;
                    transformStamped.header.frame_id = "map";
                    transformStamped.child_frame_id = "odom";
                    transformStamped.transform.translation.x = t_mo.x();
                    transformStamped.transform.translation.y = t_mo.y();
                    transformStamped.transform.translation.z = t_mo.z();
                    transformStamped.transform.rotation.x = q_mo.x();
                    transformStamped.transform.rotation.y = q_mo.y();
                    transformStamped.transform.rotation.z = q_mo.z();
                    transformStamped.transform.rotation.w = q_mo.w();
                    tf_broadcaster->sendTransform(transformStamped);
                    }
                    break;
            }
