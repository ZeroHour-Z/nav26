...
def generate_launch_description():
    ...
    # PCD保存参数（默认路径相对本包根目录下的 tmp/，与 slam_mapping_only 一致）
    ...
    _launch_file = Path(__file__).resolve()
    _pkg_root = _launch_file.parent.parent
    _tmp = _pkg_root / "tmp"
    _tmp.mkdir(parents=True, exist_ok=True)
    _ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    default_src_pcd_path = str(_tmp / f"pcd_{_ts}.pcd")
