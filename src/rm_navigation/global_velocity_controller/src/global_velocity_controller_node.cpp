...
public:
    SimplifiedControllerNode():
        ...
        declare_parameter<double>("cmd_decel_limit_linear", 4.0);  // 减速率，全向轮需大于加速率防打滑
        declare_parameter<double>("cmd_accel_limit_angular", 4.0);
        declare_parameter<double>("velocity_deadband", 0.02);      // 低于此速度直接归零
        declare_parameter<double>("stop_speed_threshold", 0.04);   // 目标速度低于此阈值立即停车

        // 路径跟踪参数
        declare_parameter<double>("goal_tolerance", 0.1);
        declare_parameter<double>("goal_decel_radius", 0.5);  // 距终点此距离开始线性减速
        declare_parameter<double>("goal_min_speed_ratio", 0.15); // 减速区最低速度比例
        declare_parameter<double>("goal_lock_duration", 0.8);   // 到达后锁定零速持续时间(s)
        ...
        // 高频里程计位置预测（补偿TF低频延迟）
        declare_parameter<std::string>("odom_highfreq_topic", "/odin1/odometry_highfreq");
        declare_parameter<bool>("pose_predict_enabled", true);
        ...
        goal_decel_radius_ = get_parameter("goal_decel_radius").as_double();
        goal_min_speed_ratio_ = get_parameter("goal_min_speed_ratio").as_double();
        goal_lock_duration_ = get_parameter("goal_lock_duration").as_double();
        ...
        cmd_decel_limit_linear_ = get_parameter("cmd_decel_limit_linear").as_double();
        ...
        // 高频里程计订阅（Odin1 ~400Hz），大幅降低定位延迟
        pose_predict_enabled_ = get_parameter("pose_predict_enabled").as_bool();
        if (pose_predict_enabled_) {
            auto odom_hf_topic = get_parameter("odom_highfreq_topic").as_string();
            odom_highfreq_sub_ = create_subscription<nav_msgs::msg::Odometry>(
                odom_hf_topic,
                rclcpp::SensorDataQoS(),
                std::bind(&SimplifiedControllerNode::onOdomHighfreq, this, _1)
            );
            RCLCPP_INFO(get_logger(), "Highfreq odom prediction enabled: %s", odom_hf_topic.c_str());
        }
...
private:
    ...
    void onOdomHighfreq(const nav_msgs::msg::Odometry::SharedPtr msg) {
        hf_odom_x_ = msg->pose.pose.position.x;
        hf_odom_y_ = msg->pose.pose.position.y;
        tf2::Quaternion q;
        tf2::fromMsg(msg->pose.pose.orientation, q);
        double r, p;
        tf2::Matrix3x3(q).getRPY(r, p, hf_odom_yaw_);
        // twist 是 body 帧速度
        hf_odom_vx_ = msg->twist.twist.linear.x;
        hf_odom_vy_ = msg->twist.twist.linear.y;
        hf_odom_stamp_ = msg->header.stamp;
        has_hf_odom_ = true;
    }
    ...
    void onControlTimer() {
        ...
        // 到达锁定：持续发零速防止惯性反弹振荡
        if (goal_locked_) {
            const double locked_elapsed = (now - goal_lock_start_).seconds();
        ...
        // 计算到最终目标的距离（不是前瞻点距离）
        const auto& final_goal = last_path_.poses.back().pose.position;
        const double dist_to_final = std::hypot(final_goal.x - current_x, final_goal.y - current_y);
        const double dist_to_lookahead = std::hypot(ex_lookahead, ey_lookahead);

        // 到达判定：使用最终目标距离
        if (dist_to_final < goal_tolerance_) {
            RCLCPP_INFO(get_logger(), "Goal reached (dist=%.3fm), locking for %.1fs",
                        dist_to_final, goal_lock_duration_);
            has_path_ = false;
            enterGoalLock();
            resetControllerState();
            return;
        ...
        // 目标减速区：接近终点时线性降速，防止全向轮惯性超调
        if (is_final_goal && dist_to_final < goal_decel_radius_) {
            // 线性衰减：从 decel_radius 到 goal_tolerance 之间，速度从 1.0 降到 min_ratio
            const double ratio = goal_min_speed_ratio_ +
                (1.0 - goal_min_speed_ratio_) *
                (dist_to_final - goal_tolerance_) /
                std::max(goal_decel_radius_ - goal_tolerance_, 0.01);
            const double scale = std::clamp(ratio, goal_min_speed_ratio_, 1.0);
            vx_out *= scale;
            vy_out *= scale;
        }

        // 停车阈值：目标速度过低时直接归零，防止全向轮滚子滑行
        if (std::hypot(vx_out, vy_out) < stop_speed_threshold_) {
            vx_out = 0.0;
            vy_out = 0.0;
        }

        // 非对称加减速限制：减速用更大的速率以对抗惯性滑行
        const double max_accel_dv = cmd_accel_limit_linear_ * dt;
        const double max_decel_dv = cmd_decel_limit_linear_ * dt;

        auto asymmetric_clamp = [](double target, double last, double accel_dv, double decel_dv) {
            double dv = target - last;
            // 判断是在加速还是减速（速度幅值是否在减小）
            bool decelerating = (std::abs(target) < std::abs(last)) || (target * last < 0);
            double max_change = decelerating ? decel_dv : accel_dv;
            if (std::abs(dv) > max_change) {
                return last + std::copysign(max_change, dv);
            }
            return target;
        };

        vx_out = asymmetric_clamp(vx_out, last_cmd_vx_, max_accel_dv, max_decel_dv);
        vy_out = asymmetric_clamp(vy_out, last_cmd_vy_, max_accel_dv, max_decel_dv);

        // Deadband：最终输出低于死区则归零，防止电机微振和滚子持续滑动
        if (std::abs(vx_out) < velocity_deadband_) vx_out = 0.0;
        if (std::abs(vy_out) < velocity_deadband_) vy_out = 0.0;
    ...
    void onEscapeTimer() {
        ...
        double vx_out = std::clamp(ux_b, -max_vx_, max_vx_);
        double vy_out = std::clamp(uy_b, -max_vy_, max_vy_);

        // 非对称加减速（与正常控制一致）
        const double max_accel_dv = cmd_accel_limit_linear_ * dt;
        const double max_decel_dv = cmd_decel_limit_linear_ * dt;
        auto asymmetric_clamp_esc = [](double target, double last, double a_dv, double d_dv) {
            double dv = target - last;
            bool decel = (std::abs(target) < std::abs(last)) || (target * last < 0);
            double mc = decel ? d_dv : a_dv;
            return (std::abs(dv) > mc) ? last + std::copysign(mc, dv) : target;
        };
        vx_out = asymmetric_clamp_esc(vx_out, last_cmd_vx_, max_accel_dv, max_decel_dv);
        vy_out = asymmetric_clamp_esc(vy_out, last_cmd_vy_, max_accel_dv, max_decel_dv);
    ...
    bool getCurrentPose(double& x, double& y, double& yaw) {
        ...
        // 优先使用高频里程计(~400Hz) + map→odom TF，大幅降低定位延迟
        if (pose_predict_enabled_ && has_hf_odom_) {
            try {
                auto tf_map_odom = tf_buffer_.lookupTransform(
                    map_frame_, "odom", tf2::TimePointZero);

                double ox = hf_odom_x_;
                double oy = hf_odom_y_;
                double oyaw = hf_odom_yaw_;

                // 速度外推：补偿从 odom 发布到当前时刻的微小间隔
                const double now_sec = get_clock()->now().seconds();
                const double odom_sec = rclcpp::Time(hf_odom_stamp_).seconds();
                const double dt_extrap = std::clamp(now_sec - odom_sec, 0.0, 0.05);
                if (dt_extrap > 1e-4) {
                    // body帧速度 → odom帧速度
                    const double cy = std::cos(oyaw);
                    const double sy = std::sin(oyaw);
                    ox += (cy * hf_odom_vx_ - sy * hf_odom_vy_) * dt_extrap;
                    oy += (sy * hf_odom_vx_ + cy * hf_odom_vy_) * dt_extrap;
                }

                // odom帧 → map帧
                const double tx = tf_map_odom.transform.translation.x;
                const double ty = tf_map_odom.transform.translation.y;
                tf2::Quaternion tf_q;
                tf2::fromMsg(tf_map_odom.transform.rotation, tf_q);
                double tr, tp, tf_yaw;
                tf2::Matrix3x3(tf_q).getRPY(tr, tp, tf_yaw);

                const double ct = std::cos(tf_yaw);
                const double st = std::sin(tf_yaw);
                x = tx + ct * ox - st * oy;
                y = ty + st * ox + ct * oy;
                yaw = normalizeAngle(oyaw + tf_yaw);
                return true;

            } catch (const tf2::TransformException& ex) {
                RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 2000,
                    "map→odom TF failed, falling back: %s", ex.what());
            }
        }
    ...
    void publishSimulatedTransform(const rclcpp::Time& now) {
        ...
        tf_msg.header.frame_id = sim_tf_parent_frame_;
    ...
    double goal_lock_duration_;
    double max_dt_;
    double kp_xy_, ki_xy_, kd_xy_, kp_yaw_;
    double max_vx_, max_vy_, max_wz_;
    double cmd_accel_limit_linear_, cmd_decel_limit_linear_, cmd_accel_limit_angular_;
    double velocity_deadband_, stop_speed_threshold_;
    ...
    // 到达锁定状态
    bool goal_locked_ { false };
    rclcpp::Time goal_lock_start_;

    // 高频里程计位置预测
    bool pose_predict_enabled_ { false };
    bool has_hf_odom_ { false };
    double hf_odom_x_ { 0.0 }, hf_odom_y_ { 0.0 }, hf_odom_yaw_ { 0.0 };
    double hf_odom_vx_ { 0.0 }, hf_odom_vy_ { 0.0 };
    rclcpp::Time hf_odom_stamp_;
